...
def main():
    ...
    vless_servers = [l for l in vless_and_rules if is_vless(l)]
    ...
    with open(OUT_ORIGINAL, "w", encoding="utf-8") as f:
        for line in vless_and_rules:
            f.write(line + "\n")
    print(f"Original: {len(vless_and_rules)} lines")

    print(f"Checking {len(vless_servers)} VLESS servers...")
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as ex:
        futs = {ex.submit(job, l): l for l in vless_servers}
    ...
    print(f"Alive: {len(alive)}, Dead: {len(results) - len(alive)}")
    ...
    with open(OUT_CHECKED, "w", encoding="utf-8") as f:
        for line, cc, num in numbered:
            if num is not None:
                ...
                f.write(line + "\n")
    print(f"Checked: {len(numbered)} lines")
