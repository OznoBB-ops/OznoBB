...
OUTPUT_DIR = "clash/output"
PROXIES_FILE = f"{OUTPUT_DIR}/proxies.txt"
CONFIG_FILE = f"{OUTPUT_DIR}/clash_config.yaml"
...
def generate_clash_config():
    """Создаёт Clash-конфиг с правилами."""
    ...
    config = {
        ...
        "proxy-providers": {
            "merged-sub": {
                "type": "http",
                "url": "https://raw.githubusercontent.com/OznoBB-ops/OznoBB/main/clash/output/proxies.txt",
                "path": "./proxy_providers/merged-sub.yml",
                "interval": 3600,
                "health-check": {
                    "enable": True,
                    "url": "http://cp.cloudflare.com/generate_204",
        ...
        "rules": [
            "RULE-SET,torrent-clients,TORRENT",
            "RULE-SET,torrent-trackers,TORRENT",
            "RULE-SET,telegram,TELEGRAM",
            "RULE-SET,personal24,PERSONAL24",
            "RULE-SET,ru-bundle,DIRECT",
            "RULE-SET,ru-custom,DIRECT",
...
def main():
    print("=" * 50)
    print("🚀 СБОРКА CLASH-КОНФИГА (Россия + Торренты + Telegram + Personal-24)")
    print("=" * 50)
    ...
    # 6. Генерируем Clash-конфиг
    print(f"\n⏳ Шаг 3: Генерация Clash-конфига...")
    ...
    print(f"   📄 Clash-конфиг: {CONFIG_FILE}")
